...
export function AuthProvider({ children }) {
    ...
    //When a new user is created, I want the todolist to be instantiated with pre-defined pictures and trainings to intro/demo the user ("Welcome to todolist, click this and that")
    async function initializeDatabase(){
        const usersCollectionRef = collection(db, "users", auth.currentUser.uid, "trainings");
        // await addDoc(usersCollectionRef, { title: "See your first training is here!", timestamp: Date.now(), checked: false });
        // await addDoc(usersCollectionRef, { title: "<- Tick this Checkbox after you completed the training!", timestamp: Date.now(), checked: false });
